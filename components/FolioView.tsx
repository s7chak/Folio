...
function FolioView({isDarkMode}) {
  ...
  const doFactSheet = async (stocks, code) => {
    let x = healthCheck();
    try {
      const response = await axios.get(envInfo[activeEnv]['api']+'/factsheet', {
      ...
      setIsLoading(false);
      console.error('Error fetching fact sheet:', error, envInfo[activeEnv]['api']);
      throw error;
    }
  }
  ...
  const handleOptimal = async () => {
    if (!!startDate) {
      try {
        let selectedStocksString = selectedStocks.length>0? selectedStocks.join(',') : 'all';
        const response = await axios.get(envInfo[activeEnv]['api']+'/fetchOptimal', {
            params: { code: code, startDate: startDate, symbols: selectedStocksString },
        });
        const data = response.data;
        alert(data['message']);
        setAnomModalVisible(true);
        setAnomImage(data['optimal']);
        setOptimalWeights(data['optimal_folio']);
        setShowWeights(true);
      } catch (error) {
        console.error('Error fetching optimal folio:', error);
        throw error;
      }
    }
  }
  ...
  const AnomalyPlotModal = ({ isVisible, imageBytes }) => {
    return (
      <Modal visible={isVisible}
        animationType='slide'
        >
        <ScrollView style={{ flex: 1, height: Dimensions.get('window').height * 3.8 }}
          contentContainerStyle={[styles.modalContainer, isDarkMode && styles.modalContainerDark]}
          showsVerticalScrollIndicator={false}
          pinchGestureEnabled={true}
          maximumZoomScale={3}
  ...
  const toggleExpandStocks = () => {
    ...
    setShowWeights(!stockExpandable && !!optimalWeights);
  };
  ...
  return (
    <ScrollView 
      ...
      {savedStocks.length>0 && !!factSheet && !isLoading && (
        <View style={styles.selectedListContainer}>
          <View style={styles.factListItem}>
            <Text style={styles.selectedListHeader}>Summary</Text>
            {Object.entries(factSheet).map(([key, val]) => (
      ...
      {savedStocks.length>0 && !!historyMetricSheet && (
        <ScrollView
        ...
        <View style={styles.metricContainer}>
          <Text style={styles.selectedListHeader}>Period Avg</Text>
          {Object.entries(historyMetricSheet['Avg']).map(([key, val]) => (
            <View key={key} style={styles.row}>
              <Text style={styles.keyText}>{key}:</Text>
              <Text style={styles.valueText}>{val}</Text>
            </View>
        ...
        <View style={styles.metricContainer}>
          <Text style={styles.selectedListHeader}>Stats</Text>
          {Object.entries(historyMetricSheet['Stats']).map(([key, val]) => (
            <View key={key} style={styles.row}>
              <Text style={styles.keyText}>{key}:</Text>
              <Text style={styles.valueText}>{val}</Text>
            </View>
      ...
      {stockExpandable && !!stockSheet && (
        <View style={styles.dateInputContainer}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text style={[styles.startLabel, isDarkMode && styles.startLabelDark]}>Start:</Text>
            <DateTimePicker
              value={startDate}
              mode="date"
              display="default"
              style={[styles.dinput, isDarkMode && styles.dinputDark]}
            ...
            <TouchableOpacity
              style={styles.button}
              onPress={handleCorrelation}
              activeOpacity={0.8}
            >
              <Text style={styles.buttonText}>Corr</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.button}
              onPress={handleAnomaly}
              activeOpacity={0.8}
            >
              <Text style={styles.buttonText}>Anoms</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.button}
              onPress={handleOptimal}
              activeOpacity={0.8}
            >
              <Text style={styles.buttonText}>Optim</Text>
            </TouchableOpacity>
            </View>
          </View>
      )}
      {corrModal && (
        <CorrelationPlotModal
          isVisible={corrModal}
          imageBytes={corrImage}
        />
      )}
      {anomModal && (
        <AnomalyPlotModal
          isVisible={anomModal}
          imageBytes={anomImage}
        />
      )}
        ...
        {savedStocks.length>0 && (
          <View style={styles.selectedListContainer}>
            ...
            {stockExpandable && !!stockSheet && (
              ...
              {showWeights && !!optimalWeights && (optimalWeights.length > 0) && (
                <View style={styles.weightContainer}>
                  {optimalWeights.map((item, index) => (
                    <View key={index} style={styles.tileContainer}>
                      <Text style={styles.tileHeader}>{item.name}</Text>
                      <Text style={styles.tileText}>Optimal: {item.weight}</Text>
                      <Text style={styles.tileText}>Percentage: {item.percentage}</Text>
                    </View>
                  ))}
                </View>
              )}
